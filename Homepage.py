...
review_tokens = [stemmer.stem(word) for word in review_tokens]
review_untokenized = ' '.join(review_tokens)
review_vector = vectorizer.transform([review_untokenized])
if svm.predict(review_vector)==1:
    st.success("The review is Positive (SVM Classifier)")
elif review=="":
    st.info("No input detected.")
else:
   st.error("The review is Negative (SVM Classifier)")
